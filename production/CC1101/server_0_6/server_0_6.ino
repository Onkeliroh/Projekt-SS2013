...
byte _clients[] = {1, 2, 3}; // 1 := LED-strip, 2 := Wheel, 3 := Accel
...
void ccHandle(byte key)
{
  ...
  switch (key)
  {
    case 27: // Wheel: "turn_left"-event
      _ccPacketHandler.buildPacket(1, 45); // new ccPacket to senders actuator with key 45
      ccSend();
      break;
    case 28: // Wheel: "turn_right"-event
      _ccPacketHandler.buildPacket(1, 46); // new ccPacket to senders actuator with key 45
      ccSend();
      break;
    case 31: // Accel: "shake"-event   
      ccAcknowledge();   
      _ccPacketHandler.buildPacket(1, 47); // new ccPacket to senders actuator with key 45
      ccSend();
