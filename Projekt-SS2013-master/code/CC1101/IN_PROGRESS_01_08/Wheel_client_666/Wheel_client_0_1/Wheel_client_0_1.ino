...
byte rawRSSI = 255;
byte neighbourID = 255;
byte prevDetecDevice = 100;

//boolean sendNeighbourRSSI = true;
byte sendNeighbourRSSI = 0;
  ...
  {    
    ...
    if(_cc1101.receiveData(&ccPacket) > 0) // some data was received      
    {
      if (ccPacket.crc_ok && ccPacket.length > 1) // the whole ccPacket was properly received
      {
        ...
        if(ccPacket.data[0] == _clientAddress)        
          ccHandle(ccPacket); // set as ccPacket and evaluate its content 
        else
        {
          rawRSSI = ccPacket.rssi;
          neighbourID = ccPacket.data[1];            
          emisorRSSI(rawRSSI,neighbourID); 
          emisorRSSI(rawRSSI,neighbourID);   
...
void emisorRSSI(byte rawRSSI,byte neighbourID)
{
    _ccPacketHandler.buildRSSIPacket(rawRSSI,neighbourID);
    ccSend();  
    _ccClear = true;    
}
...
void ccSendWheelDirection()
{
  ...
  {
    case 1: // right
      _ccPacketHandler.buildPacket(1, 28);
      break;
    case 2: // left
      _ccPacketHandler.buildPacket(1, 27);
      break;
    default:
      break;
  }
